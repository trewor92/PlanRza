function tree(id, url) {
	...
	function load(node) {
		...
		function getUrl(node) {
			var data = node.dataset.method;
			if (data)
				return url +'/'+ data +'/' + (node.dataset.params?node.dataset.params:'');
			else return null;
		}
		...
		function onLoaded(data) {
			for (var i = 0; i < data.length; i++) {
				...
				if (child.params) li.dataset.params = child.params
				if (child.method) li.dataset.method=child.method
				li.className = "Node Expand" + (child.isFolder ? 'Closed' : 'Leaf')
				if (i == data.length - 1) li.className += ' IsLast'

				li.innerHTML = '<div class="Expand"></div><div class="Content">' + child.name + '</div>'
				if (child.isFolder) {
